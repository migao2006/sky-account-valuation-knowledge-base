...
class VendorCatalogImportTests(unittest.TestCase):
    ...
    def test_crosswalk_is_deterministic_and_does_not_promote_canonical_items(self):
        ...
        self.assertEqual(summary["canonical_matched_count"] + summary["candidate_matched_count"] + summary["unmatched_collectible_count"], 1758)
        ...
        self.assertEqual(summary["canonical_matched_count"], sum(row["match_status"] == "matched_canonical_name" for row in rows))
        self.assertEqual(summary["unmatched_collectible_count"], sum(row["match_status"] == "unmatched_vendor_item" for row in rows))
