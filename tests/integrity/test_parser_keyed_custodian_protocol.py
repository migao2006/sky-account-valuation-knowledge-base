...
class KeyedCustodianProtocolTests(unittest.TestCase):
    ...
    def _contract(self) -> tuple[Path, Path, Path, Path, str]:
        assignments = [
        ...
        authority = self.external / "custodian-authorities.json"
        authority.write_bytes(canonical_bytes({"schema_version": "sky-parser-keyed-custodian-authority-bundle-v1", "authorities": [{"authority_id": "parser_custodian_authority_fixture", "public_key": public, "fingerprint": fingerprint, "roles": ["keyed_custodian_contract"]}], "revoked_fingerprints": []}))
        contract = {
            "schema_version": "1.0-p3.7", "contract_type": "parser_review_keyed_custodian_contract", "cohort_id": "parser_keyed_fixture_20260817", "keyed_protocol": "sky-parser-review-keyed-hmac-v1",
            ...
            "custodian_id": "parser_custodian_fixture", "authority_id": "parser_custodian_authority_fixture", "fingerprint": fingerprint, "signature_file": "contract.sig",
        }
        ...
        return path, ledger, packets, authority, digest(authority.read_bytes())

    def test_contract_publishes_non_linkable_manifest_and_issues_packets(self):
        contract, ledger, packets, authority, authority_sha = self._contract()
        parsed = json.loads(contract.read_text()); self.assertEqual(validate_keyed_custodian_contract(parsed, contract, self.release, authority, authority_sha), parsed)
        manifest = publish_keyed_queue_manifest(contract, self.release / "data/review/parser-gold/review-queue-manifest.json", self.release, authority, authority_sha)
        ...
        result = issue_keyed_blind_packages(contract, ledger, packets, self.external / "handoff", self.release, authority, authority_sha)
    ...
    def test_unsigned_or_linkable_contract_or_packet_is_rejected(self):
        contract, ledger, packets, authority, authority_sha = self._contract(); data = json.loads(contract.read_text())
        signature = (self.external / "contract.sig").read_bytes(); (self.external / "contract.sig").unlink()
        with self.assertRaises(ValueError): publish_keyed_queue_manifest(contract, self.external / "manifest.json", self.release, authority, authority_sha)
        ...
        with self.assertRaises(ValueError): issue_keyed_blind_packages(contract, ledger, packets, self.external / "handoff", self.release, authority, authority_sha)

    def test_contract_cannot_self_root_or_use_revoked_authority(self):
        contract, _ledger, _packets, authority, authority_sha = self._contract()
        data = json.loads(contract.read_text())
        data["public_key"] = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFakeSelfRoot"
        self.assertRaises(ValueError, validate_keyed_custodian_contract, data, contract, self.release, authority, authority_sha)
        bundle = json.loads(authority.read_text())
        bundle["revoked_fingerprints"] = [bundle["authorities"][0]["fingerprint"]]
        authority.write_bytes(canonical_bytes(bundle))
        self.assertRaises(ValueError, validate_keyed_custodian_contract, json.loads(contract.read_text()), contract, self.release, authority, digest(authority.read_bytes()))
