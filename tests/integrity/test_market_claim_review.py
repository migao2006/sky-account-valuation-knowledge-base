...
from build_market_claim_review import OPAQUE_BUCKETS, REQUESTED_FIELDS, build_queue, read_jsonl, validate_gold_links
...
class MarketClaimReviewTests(unittest.TestCase):
    ...
    def test_committed_queue_is_fixed_stratified_and_anonymous(self):
        ...
        self.assertEqual(len({row["listing_id"] for row in queue}), 20)
        required_ids = {"listing_0260", "listing_0388", "listing_0708", "listing_0792", "listing_0864"}
        self.assertTrue(required_ids <= {row["listing_id"] for row in queue})
        self.assertGreaterEqual(len({row["selection_bucket"] for row in queue}), 8)
        self.assertTrue(all(row["selection_bucket"] in set(OPAQUE_BUCKETS.values()) for row in queue))
