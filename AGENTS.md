...
- P2.1 is not a full-item or trained-model release. Vendor-correlated template candidates are single-source review evidence, not two-source identity confirmation, canonical verification, or model eligibility. Do not claim completion of all items, image evidence, visual references, verified sales, Item Value Table values, or model estimates while the formal artifacts are `insufficient_training_data`.
