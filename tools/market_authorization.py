...
def verify_authorized_market_intake(root: Path, authority_bundle: str|Path|None=None, authority_bundle_sha256: str|None=None, statement: str|Path|None=None, statement_sha256: str|None=None) -> list[str]:
    ...
    for ds in datasets:
        ...
        if not isinstance(m,dict) or m.get("schema_version")!="authorized-market-manifest-v1" or m.get("dataset_id")!=did: errors.append(f"{did}: manifest identity/schema is invalid"); continue
...
class AuthorizedMarketEvaluator:
    ...
    def __call__(self,row: dict[str,Any]) -> bool:
        ...
        if self.errors:
            return False
        observation = next((value for key, value in self.authorized_observations if key == required), None)
...
def make_authorization_evaluator(root: Path, authority_bundle: str|Path|None=None, authority_bundle_sha256: str|None=None, statement: str|Path|None=None, statement_sha256: str|None=None) -> AuthorizedMarketEvaluator:
    ...
    if not errors:
        for ds in _jsonl(root/REGISTRY_REL):
            ...
            mappings.extend(
                ((str(ds["authorization_record_id"]), str(ds["dataset_id"]), str(row["observation_id"]), sha256_bytes(canonical_bytes(row)), str(ds["manifest_sha256"]).upper()), row)
                for row in rows
            )
    return AuthorizedMarketEvaluator(tuple(mappings),tuple(errors))
...
def model_training_authorization_reasons(row: dict[str,Any], external_evaluator: Callable[[dict[str,Any]],bool]|None=None) -> list[str]:
    ...
    if x.get("status")=="authorized_model_training":
        if isinstance(external_evaluator, AuthorizedMarketEvaluator) and external_evaluator.feature_lineage_bound is not True:
            r.append("market_data_feature_lineage_evaluator_required")
        elif not(callable(external_evaluator) and external_evaluator(row) is True):
