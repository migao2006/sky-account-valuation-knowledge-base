...
ALLOWED_VERIFIER_IDS = frozenset({"nintendo_starter_pack", "aurora_faq968", "journey_pack", "moomin_pack", "kizuna_ai_bundle", "skyfest_faq1330_core_five", "tournament_of_triumph_faq1330_core_four", "days_of_color_faq1323_core_three"})
...
def validate_registry(
    ...
    for row in load_registry(root):
        ...
        for item_id in target_item_ids:
            ...
            if not item or item.get("verification_status") != "verified" or item.get("model_feature_status") != "excluded_pending_verification":
                problems.append(f"{prefix}: item target must be verified and model excluded")
