#!/usr/bin/env python3
"""Replay the bounded FAQ 1343 Days of Sunlight 2024 core-three cohort."""
...
OFFICIAL_SOURCE = "source_tgc_faq_1343_days_of_sunlight_core_three"
SECONDARY_SOURCE = "source_skygame_data_1_3_4"
OFFICIAL_LINEAGE = "lineage_tgc_support_faq_1343"
SECONDARY_LINEAGE = "lineage_skygame_data_1_3_4"
OFFICIAL_PATH = "data/source/research/tgc-faq-1343-days-of-sunlight-core-three.json"
SECONDARY_PATH = "data/source/vendor/skygame-data-1.3.4-items.json"
OFFICIAL_SNAPSHOT_SHA256 = "459E4162C304670655E3D99617D1AB3D52E8F26300473BB52F9DEC6007CD368D"
SECONDARY_SNAPSHOT_SHA256 = "21CCAD77006C425B27EE9314870BB5BB77E8436459C6DA214ABCB2B0D8329BBB"
AS_OF = "2026-08-17"

# Item id, pinned vendor numeric id/GUID/title, exact FAQ wording, category/type, cost.
ITEMS = (
    ("item_days_of_sunlight_manta_float", 2291, "WHZ443yFkG", "Sunlight Manta Float", "Sunlight Manta Float", "furniture", "Furniture", "event_currency", 20),
    ("item_days_of_sunlight_helios_hoops", 2290, "lv_MnKorJN", "Sunlight Helios Hoops", "Sunlight Helios Hoops earrings", "accessory", "HeadAccessory", "USD", 2.99),
    ("item_days_of_sunlight_woven_wrap", 2289, "vNSMly5D9b", "Sunlight Woven Wrap", "Sunlight Woven Wrap cape", "cape", "Cape", "USD", 14.99),
)


class DaysOfSunlightEvidenceError(ValueError):
    pass


def registry_contract() -> dict[str, object]:
    return {
        "cohort_id": "canonical_cohort_days_of_sunlight_faq1343_core_three",
        "evidence_path": "data/review/days-of-sunlight-faq1343-core-three-canonical-evidence.jsonl",
...
def item_row(item_id: str, official_name: str, category: str, currency: str, cost: int | float) -> dict[str, Any]:
    suffix = item_id.removeprefix("item_days_of_sunlight_")
    return {
        "item_id": item_id, "canonical_name_zh_tw": f"待確認（{official_name}）", "canonical_name_en": official_name,
        "aliases": [], "item_category": category, "item_subcategory": "days_of_sunlight_2024_historical_item",
        ...
        "availability_event_ids": ["availability_days_of_sunlight_faq1343_" + suffix], "visual_reference_ids": [],
        ...
        "notes": "FAQ 1343 establishes this named Days of Sunlight 2024 item's historical cost and event window only; the pinned vendor snapshot independently supplies title, GUID, and vendor type. Current availability, return policy, permanent-account property, formal Traditional Chinese name, visual identity, first release date, and model eligibility remain unknown or unasserted. This is a bounded three-item FAQ slice; Sunlight Beach Shorts outfit is deliberately excluded, and no bundle or complete Days of Sunlight catalog is asserted.",
    }


def _vendor_title_attests(official_name: str, vendor_name: str) -> bool:
    """FAQ wording may append a generic item-kind noun absent from vendor title."""
    return official_name == vendor_name or official_name in {vendor_name + " earrings", vendor_name + " cape"}
...
def build(root: Path) -> tuple[dict[str, list[dict[str, Any]]], list[dict[str, Any]]]:
    ...
    if sha(official_bytes) != OFFICIAL_SNAPSHOT_SHA256 or sha(secondary_bytes) != SECONDARY_SNAPSHOT_SHA256:
        raise DaysOfSunlightEvidenceError("official or secondary snapshot hash mismatch")
    official, secondary = json.loads(official_bytes), json.loads(secondary_bytes)
    expected_facts = [{"item_id": item_id, "official_name_en": official_name, "original_currency": currency, "original_cost": cost} for item_id, _id, _guid, _vendor_name, official_name, _category, _type, currency, cost in ITEMS]
    facts = official.get("facts", {})
    if (official.get("source_id") != OFFICIAL_SOURCE or facts.get("new_items") != expected_facts
            or (facts.get("historical_window_start_date"), facts.get("historical_window_end_date")) != ("2024-08-26", "2024-09-08")
            or not any("Sunlight Beach Shorts outfit" in claim for claim in official.get("non_claims", []))):
        raise DaysOfSunlightEvidenceError("official FAQ 1343 contract changed")
    ...
    for source_id, source_type, lineage in ((OFFICIAL_SOURCE, "official_support", OFFICIAL_LINEAGE), (SECONDARY_SOURCE, "community_database", SECONDARY_LINEAGE)):
        ...
        if source is None or source.get("source_type") != source_type or source.get("source_lineage_id") != lineage:
            raise DaysOfSunlightEvidenceError(f"source registry or lineage mismatch: {source_id}")
    ledger: list[dict[str, Any]] = []
    for fact_index, (item_id, vendor_id, vendor_guid, vendor_name, official_name, category, vendor_type, currency, cost) in enumerate(ITEMS):
        vendor_index, vendor_row = vendor(secondary, vendor_id)
        if vendor_row.get("guid") != vendor_guid or vendor_row.get("name") != vendor_name or vendor_row.get("type") != vendor_type:
            raise DaysOfSunlightEvidenceError(f"secondary identity changed: {vendor_id}")
        if not _vendor_title_attests(official_name, vendor_name):
            raise DaysOfSunlightEvidenceError(f"official and vendor title relation changed: {vendor_id}")
        prefix = f"/facts/new_items/{fact_index}"
        ledger.extend((
            evidence("item", item_id, "canonical_name_en", official_name, OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, prefix + "/official_name_en", "independent_identity", "FAQ 1343 exact item wording; vendor identity is retained separately."),
            ...
            evidence("item", item_id, "original_cost", cost, OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, prefix + "/original_cost", "independent_field", "Historical FAQ cost only; it is not a resale value."),
            evidence("item", item_id, "availability_history", facts["historical_window_start_date"], OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, "/facts/historical_window_start_date", "independent_field", "Historical Days of Sunlight window start only; current availability stays unknown."),
            evidence("item", item_id, "availability_history", facts["historical_window_end_date"], OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, "/facts/historical_window_end_date", "independent_field", "Historical Days of Sunlight window end only; current availability stays unknown."),
            evidence("item", item_id, "vendor_item_name", vendor_name, SECONDARY_SOURCE, SECONDARY_LINEAGE, "secondary_reference", SECONDARY_PATH, secondary_bytes, f"/items/{vendor_index}/name", "secondary_field", "Pinned vendor title independently locates the FAQ item; generic item-kind wording is retained only in the official canonical title."),
...
def apply_targets(targets: dict[str, list[dict[str, Any]]]) -> dict[str, list[dict[str, Any]]]:
    ...
    for item_id, _vendor_id, _guid, _vendor_name, official_name, category, _vendor_type, currency, cost in ITEMS:
...
def availability_rows() -> list[dict[str, Any]]:
    return [{"availability_id": "availability_days_of_sunlight_faq1343_" + item_id.removeprefix("item_days_of_sunlight_"), "item_id": item_id, "availability_status": "limited_time", "start_date": "2024-08-26", "end_date": "2024-09-08", "event_id": None, "source_ids": [OFFICIAL_SOURCE], "last_verified_at": AS_OF, "verification_status": "verified"} for item_id, *_ in ITEMS]
...
def verify(root: Path, require_applied: bool = True) -> list[str]:
    ...
    if require_applied:
        ...
        ledger_path = root / "data/review/days-of-sunlight-faq1343-core-three-canonical-evidence.jsonl"
...
def main() -> None:
    ...
    if args.apply:
        ...
        write(root / "data/review/days-of-sunlight-faq1343-core-three-canonical-evidence.jsonl", ledger)
