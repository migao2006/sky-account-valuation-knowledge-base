#!/usr/bin/env python3
"""Replay the bounded FAQ 1323 Days of Color 2024 core-three cohort."""
...
OFFICIAL_LINEAGE = "lineage_tgc_support_faq_1323"
SECONDARY_LINEAGE = "lineage_skygame_data_1_3_4"
OFFICIAL_PATH = "data/source/research/tgc-faq-1323-days-of-color-core-three.json"
SECONDARY_PATH = "data/source/vendor/skygame-data-1.3.4-items.json"
OFFICIAL_SNAPSHOT_SHA256 = "A699A55239D835D277A04569D80ED7261CBB6246E9476134572EFAEF7CCCB920"
SECONDARY_SNAPSHOT_SHA256 = "21CCAD77006C425B27EE9314870BB5BB77E8436459C6DA214ABCB2B0D8329BBB"
AS_OF = "2026-08-17"

# item id, vendor numeric id, GUID, FAQ title, category, vendor type, currency, historical cost
ITEMS = (
    ("item_days_of_color_dark_rainbow_mask", 2211, "NdTO2GQkMc", "Dark Rainbow Mask", "mask", "Mask", "event_currency", 32),
    ("item_days_of_color_color_glam_cut", 2212, "xN5Lftk7mA", "Color Glam Cut", "hair", "Hair", "event_currency", 18),
    ("item_days_of_color_color_bubble_machine", 2213, "vLeyYAngWk", "Color Bubble Machine", "prop", "Prop", "USD", 14.99),
)


class DaysOfColorEvidenceError(ValueError):
    pass


def registry_contract() -> dict[str, object]:
    return {
        "cohort_id": "canonical_cohort_days_of_color_faq1323_core_three",
        "evidence_path": "data/review/days-of-color-faq1323-core-three-canonical-evidence.jsonl",
...
def item_row(item_id: str, name: str, category: str, currency: str, cost: int | float) -> dict[str, Any]:
    suffix = item_id.removeprefix("item_days_of_color_")
    return {
        "item_id": item_id, "canonical_name_zh_tw": f"待確認（{name}）", "canonical_name_en": name,
        "aliases": [], "item_category": category, "item_subcategory": "days_of_color_2024_historical_item",
...
def build(root: Path) -> tuple[dict[str, list[dict[str, Any]]], list[dict[str, Any]]]:
    ...
    if sha(official_bytes) != OFFICIAL_SNAPSHOT_SHA256 or sha(secondary_bytes) != SECONDARY_SNAPSHOT_SHA256:
        raise DaysOfColorEvidenceError("official or secondary snapshot hash mismatch")
    ...
    if (official.get("source_id") != OFFICIAL_SOURCE or facts.get("new_items") != expected_facts
            or (facts.get("historical_window_start_date"), facts.get("historical_window_end_date")) != ("2024-06-24", "2024-07-07")):
        raise DaysOfColorEvidenceError("official FAQ 1323 contract changed")
    ...
    for source_id, source_type, lineage in ((OFFICIAL_SOURCE, "official_support", OFFICIAL_LINEAGE), (SECONDARY_SOURCE, "community_database", SECONDARY_LINEAGE)):
        ...
        if source is None or source.get("source_type") != source_type or source.get("source_lineage_id") != lineage:
            raise DaysOfColorEvidenceError(f"source registry or lineage mismatch: {source_id}")
    ...
    for fact_index, (item_id, vendor_id, vendor_guid, official_name, category, vendor_type, currency, cost) in enumerate(ITEMS):
        ...
        if vendor_row.get("guid") != vendor_guid or vendor_row.get("name") != official_name or vendor_row.get("type") != vendor_type:
            raise DaysOfColorEvidenceError(f"secondary identity changed: {vendor_id}")
        prefix = f"/facts/new_items/{fact_index}"
        ledger.extend((
            evidence("item", item_id, "canonical_name_en", official_name, OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, prefix + "/official_name_en", "independent_identity", "FAQ 1323 exact item wording; vendor identity is retained separately."),
            ...
            evidence("item", item_id, "availability_history", facts["historical_window_start_date"], OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, "/facts/historical_window_start_date", "independent_field", "Historical Days of Color window start only; current availability stays unknown."),
            evidence("item", item_id, "availability_history", facts["historical_window_end_date"], OFFICIAL_SOURCE, OFFICIAL_LINEAGE, "official_item_specific", OFFICIAL_PATH, official_bytes, "/facts/historical_window_end_date", "independent_field", "Historical Days of Color window end only; current availability stays unknown."),
            evidence("item", item_id, "vendor_item_name", official_name, SECONDARY_SOURCE, SECONDARY_LINEAGE, "secondary_reference", SECONDARY_PATH, secondary_bytes, f"/items/{vendor_index}/name", "secondary_field", "Pinned vendor title casefold-matches the official identity; FAQ 1323 controls canonical title spelling."),
...
def availability_rows() -> list[dict[str, Any]]:
    return [{"availability_id": "availability_days_of_color_faq1323_" + item_id.removeprefix("item_days_of_color_"), "item_id": item_id, "availability_status": "limited_time", "start_date": "2024-06-24", "end_date": "2024-07-07", "event_id": None, "source_ids": [OFFICIAL_SOURCE], "last_verified_at": AS_OF, "verification_status": "verified"} for item_id, *_ in ITEMS]
...
def verify(root: Path, require_applied: bool = True) -> list[str]:
    ...
    if require_applied:
        ...
        ledger_path = root / "data/review/days-of-color-faq1323-core-three-canonical-evidence.jsonl"
