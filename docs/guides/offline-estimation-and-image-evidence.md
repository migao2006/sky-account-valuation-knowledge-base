...
正式市場資料目前只有兩筆正常刊登同時確認 TWD、國際服與單一價格語義；唯一急售觀察含仲，另一筆 56,000 元觀察有分期加價，均保留為 `needs_review`，不能代表可訓練的帳號單價。因此估價適用範圍有限，不能代表完整市場。
...
相同圖片、同頁重複與圖像重疊以 `image_sha256`、`page_group_id`、`duplicate_of_image_sha256`、`overlaps_detection_ids` 記錄。P2.9 仍只有來源文字描述，沒有 Kizuna／Moomintroll／Journey 圖片資產、OCR 或圖示辨識結果，也沒有足夠真實標註資料可宣稱辨識準確率。文案 Item Vector 與 review-only lexical sidecar 都不是 OCR 或圖片模型。
