...
相同圖片、同頁重複與圖像重疊以 `image_sha256`、`page_group_id`、`duplicate_of_image_sha256`、`overlaps_detection_ids` 記錄。P2.1 沒有將圖片變成自動辨識能力，也沒有足夠真實標註視覺資料可宣稱辨識準確率；全物品、visual references 與圖片 evidence 尚未完成。文案 Item Vector 是離線字典解析，不是 OCR 或圖片模型。
