...
相同圖片、同頁重複與圖像重疊以 `image_sha256`、`page_group_id`、`duplicate_of_image_sha256`、`overlaps_detection_ids` 記錄。P2.6 只有十筆 Nintendo／AURORA 來源文字描述，沒有圖片資產、OCR 或圖示辨識結果，也沒有足夠真實標註資料可宣稱辨識準確率。文案 Item Vector 是離線字典解析，不是 OCR 或圖片模型。
